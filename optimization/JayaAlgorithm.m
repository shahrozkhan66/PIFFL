function [OverallBestDesign, BestCostValue, cost_iterations] = JayaAlgorithm(model, inp_population, designSpace, numberOfIternations, CovMatrix_sort)

%--- this is used in checking waterline contraint
Samples = readmatrix('/Users/shahrozkhan/Documents/Dr Diez Files/PCA_variables_temp.xlsx');
%------------------------------------
...
for n=1:numberOfIternations
    ...
    for i = 1:size(inp_population,1)
        ...
        %------check the water line constraints here -----------------
        porjectedNnorm = train_population(i,:)*CovMatrix_sort';
        for b=1:length(porjectedNnorm)
            porjectedNnorm(b) =(porjectedNnorm(b)*std(Samples(:,b))) + mean(Samples(:,b));
        end
        design3D = creatingOrinalDesign(porjectedNnorm,... % its a 3 colum design
        '/Users/shahrozkhan/Documents/Dr Diez Files/PCA_eigenvector.xlsx',...
        '/Users/shahrozkhan/Documents/Dr Diez Files/orignaldesign.xlsx');
        if (max(design3D((find(design3D(:,3)<6.15)),2)) > 9.8089)
            train_population(i,:) = create_initPop(designSpace,1);
        end
        %-------------------------------------------------------------
